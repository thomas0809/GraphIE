...
def evaluate(output_file, score_file, evaluate_raw_format=False, o_tag='O'):
    if evaluate_raw_format:
        os.system("examples/eval/conll00eval.pl -r -o %s < %s > %s" % (o_tag, output_file, score_file))
    else:
        os.system("examples/eval/conll00eval.pl -o %s < %s > %s" % (o_tag, output_file, score_file))
