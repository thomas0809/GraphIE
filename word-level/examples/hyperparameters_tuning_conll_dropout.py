...
parameters = [hidden_sizes, p_ems, p_ins, p_rnns, p_outs, dropouts, char_methods]
...
for param in params:
	hidden_size, p_em, p_in, p_rnn, p_out, dropout, char_method = param
	result_file_path = 'results/hyperparameters_tuning_activation_regularization_lstm_{}_{}_{}'.format(char_method, dataset_name, group_idx)
	p_rnn = '{} {}'.format(p_rnn[0], p_rnn[1])
	with open(result_file_path, 'a') as ofile:
		ofile.write('\nhidden_size: {}\tp_em: {}\tp_in: {}\tp_rnn: {}\tp_out: {}\tdropout: {}\tchar_method: {}\n'.format(hidden_size, p_em, p_in, p_rnn, p_out, dropout, char_method))
	print('\nhidden_size: {}\tp_em: {}\tp_in: {}\tp_rnn: {}\tp_out: {}\tdropout: {}\tchar_method: {}'.format(hidden_size, p_em, p_in, p_rnn, p_out, dropout, char_method))
	command = 'CUDA_VISIBLE_DEVICES={} python examples/NERCRF_conll.py --cuda --mode LSTM --encoder_mode lstm --char_method {} --num_epochs 200 --batch_size 16 --hidden_size {} --num_layers 1 \
				 ...
				 --test "data/conll2003/eng.test.bio.conll.simple"'.format(gpu_idx, char_method, hidden_size, gpu_idx, dropout, p_em, p_in, p_rnn, p_out, result_file_path)
	os.system(command)
