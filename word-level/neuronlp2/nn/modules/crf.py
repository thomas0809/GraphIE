...
class ChainCRF(nn.Module):
    ...
    # def decode(self, input, mask=None, lengths=None, leading_symbolic=0, kbest=1):
    #     """
    ...
    #     if kbest > 1:
    #         back_pointer = batch_index.new_zeros(length, batch_size, kbest)
    #     else:
    ...
    #     else:
    #         # print(pi[-1])
    #         pi_final_kbest, back_pointer[-1] = torch.topk(pi[-1], kbest, dim=1)
    #         for k in range(kbest):
    #             for t in reversed(range(length - 1)):
    #                 pointer_last = pointer[t + 1]
    #                 back_pointer[t, batch_index, k] = pointer_last[batch_index, back_pointer[t + 1, batch_index, k]]

    #         return back_pointer.transpose(0, 1) + leading_symbolic, nn.functional.softmax(pi_final_kbest, dim=1)
    ...
    def decode(self, input, mask=None, lengths=None, leading_symbolic=0, kbest=1):
        ...
        if kbest <= 1:
            ...
            pi_final_kbest = torch.FloatTensor(batch_size, kbest).fill_(0.)
            for i in range(batch_size):
                pi_final_kbest[i], back_pointer[lengths[i]-1, i] = torch.topk(pi[lengths[i]-1, i], kbest, dim=0)
                for k in range(kbest):
                    for t in reversed(range(lengths[i] - 1)):
                        pointer_last = pointer[t + 1, i]
                        back_pointer[t, i, k] = pointer_last[back_pointer[t + 1, i, k]]
