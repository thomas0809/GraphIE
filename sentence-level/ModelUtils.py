...
class RNN(nn.Module):
    ...
    def __init__(self, d_input, d_hidden=512, n_layers=1, cell_type='LSTM', 
        pooling=None, dropout=0, cuda=True):
        ...
        if cell_type == 'GRU':
            self.rnn = nn.GRU(d_input, d_hidden//2, n_layers, bidirectional=True, batch_first=True)
        elif cell_type == 'LSTM':
            self.rnn = nn.LSTM(d_input, d_hidden//2, n_layers, bidirectional=True, batch_first=True)
